...
public class OrderController {
    ...
    public ResponseEntity<Order> getById(@PathVariable Long id) {
        Optional<Order> result = ordersService.getById(id);
    ...
    public ResponseEntity<List<OrderDetails>> getDetails(@PathVariable Long id) {
        return ResponseEntity.ok(ordersService.getDetailsByOrder(id));
    }
 
    // GET /api/orders/user/{userId}
    @GetMapping("/user/{userId}")
    public ResponseEntity<List<Order>> getByUser(@PathVariable Long userId) {
        return ResponseEntity.ok(ordersService.getByUser(userId));
    }
 
    // POST /api/orders
    @PostMapping
    public ResponseEntity<Order> create(@RequestBody CreateOrderRequest request) {
        Optional<Order> result = ordersService.create(request);
        return result.map(o -> ResponseEntity.status(HttpStatus.CREATED).body(o))
                .orElse(ResponseEntity.badRequest().build());
    }
 
    // PUT /api/orders/{id}  → solo actualiza el estado
    @PutMapping("/{id}")
    public ResponseEntity<Order> update(
            @PathVariable Long id,
            @RequestBody UpdateOrderRequest request) {
        Optional<Order> result = ordersService.update(request, id);
        return result.map(ResponseEntity::ok)
                .orElse(ResponseEntity.notFound().build());
    }
