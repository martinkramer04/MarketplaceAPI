...
import { fetchSolicitations, updateSolicitation } from '../../../../redux/providerSolicitationSlice'
...
function Providers() {
    ...
    const { solicitations, loading } = useSelector(state => state.providerSolicitations)
    ...
    const handleSuspend = async (id) => {
        try {
            await dispatch(updateSolicitation({ id, payload: { solicitationStatus: 'RECHAZADA', status: 'REJECTED' } })).unwrap()
            toast.success(`Solicitud #${id} rechazada correctamente.`)
        } catch (err) {
            toast.error(`Error al rechazar: ${typeof err === 'string' ? err : err?.message || 'Error desconocido'}`)
        }
    }

    const handleApprove = async (id) => {
        try {
            await dispatch(updateSolicitation({ id, payload: { solicitationStatus: 'CONFIRMADA', status: 'APPROVED' } })).unwrap()
            toast.success(`Solicitud #${id} aprobada con éxito!`)
        } catch (err) {
            toast.error(`Error al aprobar: ${typeof err === 'string' ? err : err?.message || 'Error desconocido'}`)
        }
    }

    if (loading && !solicitations.length) {
        return <div style={{ padding: '2rem' }}>Cargando solicitudes desde la base de datos...</div>
    }
                    ...
                    <tbody>
                        {filtered.map((p) => (
                            <tr key={p.id}>
                                <td>#{p.id}</td>
                                <td>
                                    <div><strong>{p.businessName || '—'}</strong></div>
                                    {p.ownerName && <div style={{ fontSize: '0.8rem', color: '#666' }}>{p.ownerName}</div>}
                                </td>
                                <td>{p.category || '—'}</td>
                                <td>{p.location || '—'}</td>
                                <td><StatusBadge status={p.estado} /></td>
                                <td>
                                    <button className="btn-prov-detail" onClick={() => setSelectedId(p.id)}>
                                        Evaluar Propuesta
                                    </button>
                                </td>
                            </tr>
